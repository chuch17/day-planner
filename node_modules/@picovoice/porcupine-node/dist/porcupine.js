...
class Porcupine {
     ...
     */
    constructor(accessKey, keywords, sensitivities, options = {}) {
        if (accessKey === null ||
            accessKey === undefined ||
            accessKey.length === 0) {
            throw new errors_1.PorcupineInvalidArgumentError(`No AccessKey provided to Porcupine`);
        }
        const { modelPath = path.resolve(__dirname, MODEL_PATH_DEFAULT), device = 'best', libraryPath = (0, platforms_1.getSystemLibraryPath)(), } = options;
        ...
        try {
            ...
            porcupineHandleAndStatus = pvPorcupine.init(accessKey, modelPath, device, keywordPaths.length, keywordPaths, sensitivities);
        }
        catch (err) {
    ...
    static listAvailableDevices(options = {}) {
        ...
        const pvPorcupine = require(libraryPath); // eslint-disable-line
        let porcupineHardwareDevicesResult = null;
        try {
        ...
        if (status !== pv_status_t_1.default.SUCCESS) {
            ...
            if (errorObject.status === pv_status_t_1.default.SUCCESS) {
                (0, errors_1.pvStatusToException)(status, 'Porcupine failed to get available devices', errorObject.message_stack);
            }
            else {
